...
class Nightshift  extends CI_Controller
{
		...
		if($method != 'POST'){
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
					...
					if ($params['staff_id_fk'] == "" || $params['date'] == "" || $params['purpose'] == "" || $params['fromtime'] == "" || $params['totime'] == "" || $params['productionvolume'] =="" || $params['client'] == "" || $params['latitude'] == "" || $params['longitude'] == ""){
						$respStatus = 400;
						$resp = array('status' => 400,'message' =>  'Fields Missing');
					}else{
						$params['date_added'] = date('Y-m-d H:i:s');
						$params['main_status'] = 0;
						//$exist = $this->common_model->checkNightshift($params);
						//if(!$exist){
		        			$resp = $this->queries->insert('hrms_nightshift',$params);
		        		/*}else{
	...
	public function getNightshiftList(){
		$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
		}else{
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
					...
					if ($params['staff_id_fk'] == ""){
						...
						$params['date_added'] = date('Y-m-d H:i:s');						
		        		$resp = $this->common1_model->getNightshiftList($params);		        		
						$data=array();
						if($resp)
						{
							foreach($resp as $res){
								$data[]=array("nightshift_id" => $res->nightshift_id, 
	...
	public function update(){
		...
		if($method != 'POST'){
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
                    $params = json_decode(file_get_contents('php://input'), TRUE);
					if ($params['nightshift_id'] == "" || $params['staff_id_fk'] == "" || $params['date'] == "" || $params['purpose'] == "" || $params['fromtime'] == "" || $params['totime'] == "" || $params['productionvolume'] =="" || $params['client'] == "" || $params['latitude'] == "" || $params['longitude'] == "") {
						$respStatus = 400;
						$resp = array('status' => 400,'message' =>  'Fields Missing');
					} else {
						$nightshift_id = $params['nightshift_id'];
						unset($params['nightshift_id']);
						/*$exist = $this->queries->updateCheckExist('menu_name',$params['menu_name'],'menu_id',$menu_id,'hrms_menu');
						if(!$exist){*/
		        			$resp = $this->queries->update('nightshift_id',$nightshift_id,'hrms_nightshift',$params);
		        		/*}else{
		        			$resp = array('status' => 409,'message' =>  'Menu Name is already exist');
		        		}*/
					}
		...
		if($method != 'POST'){
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
					...
					if ($params['nightshift_id'] == "") {
						$respStatus = 400;
						$resp = array('status' => 400,'message' =>  'Fields Missing');
					} else {
						$nightshift_id = $params['nightshift_id'];
						unset($params['nightshift_id']);
						$getStaffdetail = $this->queries->query('hrms_staffmaster',array('id'=>get_val('staff_id_fk','nightshift_id',$nightshift_id,'hrms_nightshift')),'row');
						if($getStaffdetail){
		        			...
		        			$resp = $this->queries->update('nightshift_id',$nightshift_id,'hrms_nightshift',$data);
		        		}else{
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
					...
					if ($params['nightshift_id'] == "") {
						...
						$result = $this->queries->update('nightshift_id',$nightshift_id,'hrms_nightshift',array('main_status'=>2));
						if($result){
    ...
    public function getMobileNightshiftList(){				
		$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
		}else{						
			...
			if($check_auth_user == true){								
	        	...
	        	if($response['status'] == 200){								
					...
					if($params['staff_id_fk'] == ""){
						...
						$params['date_added'] = date('Y-m-d H:i:s');												
		        		$result = $this->common1_model->getMobileNightshiftList($params);
						$resp=array();		        		
		        		if($result){
                            ...
                            $status=current_status($res->fwd_4_status,$res->fwd_3_status,$res->fwd_2_status,$res->fwd_1_status,$res->fwd4,$res->fwd3,$res->fwd2,$res->fwd1);																														 
								...
								$resp[]=array("nightshift_id" => $res->nightshift_id,
                                "name" => $res->name,
								"emp_code" => $res->biometricAccess,
								"profileimage" => $res->profileimage,								
								"date" => $res->date,
    ...
    public function getApproveNightshiftList(){		
		$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
		}else{						
			...
			if($check_auth_user == true){								
	        	...
	        	if($response['status'] == 200){								
					...
					if($params['staff_id_fk'] == ""){
						...
						$params['date_added'] = date('Y-m-d H:i:s');												
		        		$result = $this->common1_model->getApproveNightshiftList($params);
                        //print_r($result);
						$resp=array();
		        		if($result){
							foreach($result as $res){								
    ...
    public function NightshiftStatusUpdate(){
		$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
		}else{
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
					...
					if ($params['nightshift_id'] == "") {
						...
						$nightshift_id = $params['nightshift_id'];
						unset($params['nightshift_id']);
					    $getStaffdetail = $this->queries->query('hrms_staffmaster',array('biometricAccess'=>get_val('staff_id_fk','nightshift_id',$nightshift_id,'hrms_nightshift')),'row');
						if($getStaffdetail)
                        {
