		...
		if($method != 'POST'){
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
					...
					if ($params['staff_id_fk'] == "" || $params['date'] == "" || $params['purpose'] == "" || $params['fromtime'] == "" || $params['totime'] == "" || $params['client'] == "" || $params['venue'] == "" || $params['location'] == "" || $params['latitude'] == "" || $params['longitude'] == ""){
						$respStatus = 400;
						$resp = array('status' => 400,'message' =>  'Fields Missing');
					}else{
						$params['date_added'] = date('Y-m-d H:i:s');
						$params['main_status'] = 0;
						//$exist = $this->common_model->checkDelegation($params);
						//if(!$exist){
		        			$resp = $this->queries->insert('hrms_delegation',$params);
		        		/*}else{
...
public function getDelegationList(){
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
					...
					if ($params['staff_id_fk'] == ""){
						...
						$params['date_added'] = date('Y-m-d H:i:s');						
		        		$resp = $this->common1_model->getDelegationList($params);		        		
						$data=array();
						if($resp)
						{
							foreach($resp as $res){
								$data[]=array("delegation_id" => $res->delegation_id, 
	...
	public function update(){
		...
		if($method != 'POST'){
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
                    $params = json_decode(file_get_contents('php://input'), TRUE);
					if ($params['delegation_id'] == "" || $params['staff_id_fk'] == "" || $params['date'] == "" || $params['purpose'] == "" || $params['fromtime'] == "" || $params['totime'] == "" || $params['client'] == "" || $params['venue'] == "" || $params['location'] == "" || $params['latitude'] == "" || $params['longitude'] == "") {
						$respStatus = 400;
						$resp = array('status' => 400,'message' =>  'Fields Missing');
					} else {
						$delegation_id = $params['delegation_id'];
						unset($params['delegation_id']);
						/*$exist = $this->queries->updateCheckExist('menu_name',$params['menu_name'],'menu_id',$menu_id,'hrms_menu');
						if(!$exist){*/
		        			$resp = $this->queries->update('delegation_id',$delegation_id,'hrms_delegation',$params);
		        		/*}else{
		        			$resp = array('status' => 409,'message' =>  'Menu Name is already exist');
		        		}*/
					}
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
					...
					if ($params['delegation_id'] == "") {
						...
						$delegation_id = $params['delegation_id'];
						unset($params['delegation_id']);
						$getStaffdetail = $this->queries->query('hrms_staffmaster',array('id'=>get_val('staff_id_fk','delegation_id',$delegation_id,'hrms_delegation')),'row');
						if($getStaffdetail){
		        			...
		        			$resp = $this->queries->update('delegation_id',$delegation_id,'hrms_delegation',$data);
		        		}else{
	...
	public function cancelrequest(){
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
					...
					if ($params['delegation_id'] == "") {
						...
						$result = $this->queries->update('delegation_id',$delegation_id,'hrms_delegation',array('main_status'=>2));
						if($result){
 	...
 	public function getMobileDelegationList(){
		$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
		}else{						
			...
			if($check_auth_user == true){								
	        	...
	        	if($response['status'] == 200){								
					...
					if($params['staff_id_fk'] == ""){
						...
						$params['date_added'] = date('Y-m-d H:i:s');												
		        		$result = $this->common1_model->getMobileDelegationList($params);
						$resp=array();		        		
		        		if($result){
                            ...
                            $status=current_status($res->fwd_4_status,$res->fwd_3_status,$res->fwd_2_status,$res->fwd_1_status,$res->fwd4,$res->fwd3,$res->fwd2,$res->fwd1);																																 
								$resp[]=array("delegation_id" => $res->delegation_id,
                                "name" => $res->name,
								"emp_code" => $res->biometricAccess,
								"profileimage" => $res->profileimage,								
								"date" => $res->date,
    ...
    public function getApproveDelegationList(){		
		$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
		}else{						
			...
			if($check_auth_user == true){								
	        	...
	        	if($response['status'] == 200){								
					...
					if($params['staff_id_fk'] == ""){
						...
						$params['date_added'] = date('Y-m-d H:i:s');												
		        		$result = $this->common1_model->getApproveDelegationList($params);
                        //print_r($result);
						$resp=array();
		        		if($result){
							foreach($result as $res){		
    ...
    public function DelegationStatusUpdate(){
		$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
		}else{
			...
			if($check_auth_user == true){
	        	...
	        	if($response['status'] == 200){
					...
					if ($params['delegation_id'] == "") {
						...
						$delegation_id = $params['delegation_id'];
						unset($params['delegation_id']);
					    $getStaffdetail = $this->queries->query('hrms_staffmaster',array('biometricAccess'=>get_val('staff_id_fk','delegation_id',$delegation_id,'hrms_delegation')),'row');
						if($getStaffdetail)
                        {
